...
#Covariate Selector
multicovariate_selector = pn.widgets.CheckButtonGroup(name='Covariates', value=['Sex', 'Race'], options=['Sex', 'Ethnicity', 'Race','Subject Group','Case_Control'])
covariate_selector = pn.widgets.RadioButtonGroup(name='Covariates', options=['None','Sex', 'Ethnicity', 'Race','Subject Group','Case_Control'], button_type='success')
...
@pn.depends(user_input,multicovariate_selector.param.value,positive_col,negative_col,participant_input)
def clustermap(gene_set,covariate,positive,negative,participant): # start function
    ...
    meta=covariates[multicovariate_selector.value]
    meta=meta[meta.index.isin(df_selected.columns)]
    net.load_df(df_selected, meta_col=meta)
    net.set_matrix_colors(pos=positive, neg=negative)
    net.cluster()
    fig=net.widget()
    return fig

@pn.depends(user_input,covariate_selector.param.value,positive_col,negative_col,participant_input)
def boxplot(gene_set,covariate,positive,negative,participant): # start function
    gene_list=list(user_input.value.split())
    df_selected=df[df['gene_id'].isin(list(gene_set.split()))]
    df_selected = df_selected.set_index('gene_id')
    df_selected = (1+df_selected)/2 # (-1,1] -> (0,1]
    ...
    if covariate_selector.value=='None':
...
sidebar_content1 = [
    pn.Column(pn.pane.Markdown("**Settings**"),user_dropdown,user_input,participant_input,multicovariate_selector, positive_col, negative_col, logout)
]
...
#material.sidebar.append(multicovariate_selector)
#material.sidebar.append(covariate_selector)
...
def load_content1(event):
    material.main[0].objects = main_content1
    material.sidebar[0].objects= sidebar_content1
    #covariate_selector = pn.widgets.CheckButtonGroup(name='Covariates', value=['Sex', 'Race'], options=['Sex', 'Ethnicity', 'Race','Subject Group','Case_Control'])


def load_content2(event):
    material.main[0].objects = main_content2
